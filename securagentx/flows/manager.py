...
# Default provider factory — builds a ConcreteFlowProvider that wires
# the FlowProvider Protocol to the securagentx agent layer (Generator /
# Refiner / Reporter / PrimaryAgent). Callers that need a custom agent
# wiring (e.g. injecting specialist handlers or a different LLM client)
# can pass their own ``provider_factory`` to FlowManager().
async def _default_provider_factory(ctx: FlowContext) -> FlowProvider:
    """Default provider factory — returns a :class:`ConcreteFlowProvider`.

    Builds a :class:`securagentx.flows.concrete_provider.ConcreteFlowProvider`
    bound to ``ctx``. The default exists so ``FlowManager()`` can be
    instantiated without an explicit ``provider_factory`` — callers that
    need custom wiring (LLM client, memory, governance, search providers)
    can either pass their own factory or wrap this one.
    """

    from securagentx.flows.concrete_provider import ConcreteFlowProvider

    return ConcreteFlowProvider(ctx)
