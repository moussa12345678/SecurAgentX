...
# NOTE: ``securagentx.api.*`` (REST API app, _auth, _models) was removed as
# dead code; only ``securagentx.auth.*`` (tokens/sessions/middleware/oauth)
# is exercised by this suite now.
...
# ---------------------------------------------------------------------------
# Auth-only fixtures (the FastAPI ``client`` fixture and its mock stores
# were removed together with ``securagentx.api.*``).
# ---------------------------------------------------------------------------
...
def session_cookie():
    ...
    The cookie is minted directly via :func:`securagentx.auth.sessions.
    create_session_cookie` using the project-wide ``TEST_SECRET`` (which
    the test conftest also surfaces as ``SECURAGENTX_SESSION_SECRET``).
    """
    ...
    cookie_value = create_session_cookie(
        user, secret_key=TEST_SECRET, ttl_seconds=3600,
    )
